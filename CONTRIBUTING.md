...
3. Declare capabilities by implementing them: `change_latent` is the minimum
   ...
   `sample_task`, `ask_confidence`, `task_fn` and `apply_intent` unlock more.
   Keep endogenous mechanisms **off by default** so scheduled experiments stay
   controlled.
...
host it is skipped rather than fatal. Seeds default to 5, the conclusive-
verdict floor; extend with `--seeds 10 --resume` when an interval straddles
...
The suite (`experiments/benchmark.py:SUITE`) is the stable yardstick. Change
it rarely and deliberately: every past matrix becomes incomparable with the
next. The research experiments evolve freely; the suite does not.
