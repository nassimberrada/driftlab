...
is skipped). Seeds default to 5, the minimum for a conclusive confidence
interval; agents default to each experiment's own arms, with --model pinning
...
from experiments.common import (DEFAULT_SEEDS, MODE, WORLD, out_dir,  # noqa: E402
                                refresh_validation, resolve_agents)
from experiments.registry import REGISTRY, regimes_needed  # noqa: E402

VERDICT_SEEDS = 5  # the min_pairs floor: fewer seeds can never produce a conclusive verdict
...
def main():
    ...
    if args.seeds < VERDICT_SEEDS and not (args.plan or args.smoke):
        print(f"note: {args.seeds} seeds cannot produce a conclusive verdict (the floor is {VERDICT_SEEDS})")
