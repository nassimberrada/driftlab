...
def check_predictions():
    ...
    for eid, entry in REGISTRY.items():
        for p in entry.get("predictions", []):
            ...
            if p.get("kind") not in ("effect", "reversal"):
            ...
            need = {"effect": ("vary", "direction"), "reversal": ("a_regime", "b_regime")}[p["kind"]]
            ...
            if regs is not None and p.get("vary") == "regime" or (regs is not None and p["kind"] == "reversal"):
...
def check_benchmark_and_agents():
    ...
    for exp in SUITE:
        ...
        if eid not in mods or eid not in REGISTRY:
            ...
            ok(f"suite: {eid} ({REGISTRY[eid]['title']})")
