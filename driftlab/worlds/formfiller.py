...
The agent submits a JSON object. Hidden validation decides which fields are
...
key names are re-labeled; the form and its rules do not change),
introduce_novelty (a new optional field with its own rule appears), probe,
ask_confidence. `version_every` schedules latent changes automatically when
a scenario does not.
