...
Because per-step feedback carries no signal, the generic accuracy metrics do
not apply here: score this world with final_reward (the reports carry the
period mean as reward) or the privileged `hit` field, which logs the truth of
every push for analysis.
...
class CampaignWorld:
    ...
    def act(self, t: int, action: str) -> tuple[float, str]:
        ...
        hit = bool(self.rng.random() < self._effective(self._chose))
        ...
        if (t + 1) % self.report_every == 0:
            ...
            self.period = []
            return mean, (f"Report: {k} of the last {n} pushes landed. No per-day breakdown is available.")
